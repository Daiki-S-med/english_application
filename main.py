...
if "messages" not in st.session_state:
    ...
    st.session_state.start_flg = False # 「英会話開始」ボタンが押されたかどうかのフラグ
    st.session_state.pre_mode = "" # 前回選択されていたモードを保存する変数
    st.session_state.shadowing_flg = False # 「シャドーイング」モードが実行されたかどうかのフラグ
    st.session_state.shadowing_button_flg = False # 「シャドーイング開始」ボタンが押されたかどうかのフラグ
    st.session_state.shadowing_count = 0 # 「シャドーイング」モードの実行回数をカウントする変数
    st.session_state.shadowing_first_flg = True # 「シャドーイング」モードが初めて実行されたかどうかのフラグ
    st.session_state.shadowing_audio_input_flg = False # 「シャドーイング」モードの音声入力が行われているかどうかのフラグ
    st.session_state.shadowing_evaluation_first_flg = True # 「シャドーイング」モードの評価処理が初めて実行されたかどうかのフラグ
    st.session_state.pre_englv = "" # 前回選択されていた英語レベルを保存する変数
    st.session_state.dictation_flg = False # 「ディクテーション」モードが実行されたかどうかのフラグ
    ...
    # 再実行のたびにAI設定を作りなおすとAPIリクエストの回数が増えてしまうため、セッションステートに保存して使いまわす
    ...
    # モード「日常英会話」用のChainは英語レベルをプロンプトに反映するため、レベル選択後（col4ブロック内）に作成する

# 初期表示
# col1, col2, col3, col4 = st.columns([1, 1, 1, 2])
# 提出課題用
col1, col2, col3, col4 = st.columns([2, 2, 3, 3]) # 横幅の比率を変更
with col1: #「英会話開始」ボタンの表示
    if st.session_state.start_flg:
        st.button("開始", use_container_width=True, type="primary") # type = "primary"を指定して、開始ボタンを強調表示
    else:
...
with col3: # モードの選択(日常会話、シャドーイング、ディクテーションから選ぶ)
    st.session_state.mode = st.selectbox(label="モード", options=[ct.MODE_1, ct.MODE_2, ct.MODE_3], label_visibility="collapsed")
    # モードを変更した際の処理
    if st.session_state.mode != st.session_state.pre_mode: #さっきまでのモード(pre_mode)と今選択されているモード(mode)が違う場合に、モード変更の処理を実行する
        ...
        # チャット入力欄を非表示にする
        st.session_state.chat_open_flg = False
    st.session_state.pre_mode = st.session_state.mode # 今回のモードを「前回のモード」としてpre_modeに記録しておく
with col4: # 英語レベルの選択
    st.session_state.englv = st.selectbox(label="英語レベル", options=ct.ENGLISH_LEVEL_OPTION, label_visibility="collapsed")
    # 英語レベルが変更された場合、またはアプリ起動直後（pre_englv=""）の場合にChainを作りなおす
    if st.session_state.englv != st.session_state.pre_englv:
        system_template = ct.SYSTEM_TEMPLATE_BASIC_CONVERSATION.format(level=st.session_state.englv)
        st.session_state.chain_basic_conversation = ft.create_chain(system_template)
        # シャドーイング/ディクテーションのChainも再作成されるようにフラグをリセット
...
if st.session_state.dictation_chat_message and not st.session_state.chat_open_flg:
    # 文字が入力されており、かつチャット入力欄が開いていない場合は、入力された文字を消す
    # （チャット入力欄は「ディクテーション」モードでAIが音声を読み上げた後に表示されるため、それ以外のタイミングで文字が入力された場合は誤入力の可能性が高いと想定されるため）
    st.stop()
...
if st.session_state.start_flg:
    ...
    # モード：「ディクテーションモード」かつ
    # 「開始ボタンが押された」か、「初回実行」か、「チャット送信時」のみ中身を実行
    if st.session_state.mode == ct.MODE_3 and (st.session_state.dictation_button_flg or st.session_state.dictation_count == 0 or st.session_state.dictation_chat_message):
        if st.session_state.dictation_first_flg:
            # 初回の場合のみ、問題文生成用のChainを作成してセッションステートに保存する。
            system_template = ct.SYSTEM_TEMPLATE_CREATE_PROBLEM.format(level=st.session_state.englv)
            st.session_state.chain_create_problem = ft.create_chain(system_template)
            st.session_state.dictation_first_flg = False
        # チャット入力以外
        # AIが問題文を生成し、音声ファイルを作成して再生する処理を実行する関数を呼び出す。
    ...
    if st.session_state.mode == ct.MODE_2 and (st.session_state.shadowing_button_flg or st.session_state.shadowing_count == 0 or st.session_state.shadowing_audio_input_flg):
        ...
        # 音声入力を受け取って音声ファイルを作成
        st.session_state.shadowing_audio_input_flg = True # シャドーイングモードの音声入力が行われている状態にするフラグを立てる
        audio_input_file_path = f"{ct.AUDIO_INPUT_DIR}/audio_input_{int(time.time())}.wav"
        ft.record_audio(audio_input_file_path)
